            ...
            {isOpen && (
                <div className="absolute top-full left-0 mt-2 z-50 bg-white rounded-xl shadow-xl border border-emerald-100 p-4 w-64 animate-in fade-in zoom-in-95">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button type="button" onClick={() => setIsOpen(false)} className="bg-emerald-600 text-white text-[10px] font-black px-3 py-2 rounded-lg hover:bg-emerald-700 uppercase">{t('ok_btn')}</button>
                    </div>
                </div>
            )}
        </div>
            ...
            const loadData = async () => {
                try {
                    const [user, reportsData] = await Promise.all([
                        fetchCurrentUser(),
                        fetchFilteredLostReports({
                            species: foundReport?.species ? [foundReport.species] : null
                        })
                    ]);
                    setCurrentUser(user);
    ...
    const handleClaim = async (lid) => {
        ...
        try {
            if (addLostReportToFoundReport) {
                await addLostReportToFoundReport(foundReport.id, lid);
            } else {
...
export const AddSightingModal = ({ isOpen, onClose, baseReportId, type = "FOUND" }) => {
    ...
    }, [isOpen, baseReportId, type]);

    useEffect(() => {
        if (isOpen && mode === "EXISTING" && baseDetails) {
            const load = async () => {
                try {
                    const [user, data] = await Promise.all([
                        fetchCurrentUser(),
                        fetchFoundReportsList({ species: [baseDetails.species] })
                    ]);
                    setCurrentUser(user);
                    setMyReports(data || []);
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white rounded-xl max-w-lg w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-4 font-bold text-xl"><h3>{t('sighting_modal_title')}</h3><button onClick={onClose}><X className="w-5 h-5 text-gray-500" /></button></div>

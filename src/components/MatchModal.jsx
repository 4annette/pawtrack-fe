...
import { X, Phone, MapPin, Calendar, FileText, Check, Loader2, AlertCircle } from "lucide-react";
...
import { fetchFoundReportById, fetchLostReportById } from "@/services/api";
...
const MatchModal = ({ notification, onClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const foundId = notification.foundReportId; 
        const lostId = notification.lostReportId;

        if (foundId && lostId) {
          const [foundData, lostData] = await Promise.all([
            fetchFoundReportById(foundId),
            fetchLostReportById(lostId)
          ]);
          setFoundReport(foundData);
          setLostReport(lostData);
        } else {
            toast.error("Error: Missing report IDs");
        }
      } catch (error) {
        console.error(error);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-emerald-900/20 backdrop-blur-md p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl shadow-emerald-900/20 overflow-hidden max-h-[90vh] flex flex-col border border-white/50 ring-4 ring-emerald-50/50">
        ...
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center p-12 gap-3">
            ...
            <p className="text-xs font-bold text-emerald-700 uppercase tracking-widest animate-pulse">Loading Details...</p>
          </div>
        ) : (
        ...
        <div className="p-5 border-t border-gray-100 bg-white flex gap-4 shrink-0">
          <button onClick={onClose} className="flex-1 py-4 rounded-2xl bg-gray-50 border border-gray-100 text-gray-500 font-black text-xs uppercase tracking-widest hover:bg-gray-100 hover:text-gray-700 transition-all active:scale-95">
            No, not mine
          </button>

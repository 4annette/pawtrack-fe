            ...
            <main className="container mx-auto px-4 py-8 max-w-5xl">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <div className="bg-white p-4 rounded-[2.5rem] border border-gray-100 shadow-xl relative">
                            <div className={`absolute top-6 left-6 z-10 px-2.5 py-1 rounded-full text-[8px] font-black uppercase tracking-tighter shadow-sm border ${report.found ? 'bg-emerald-500 text-white border-emerald-400' : 'bg-orange-500 text-white border-orange-400'}`}>
                                {report.found ? (t('status_found') || "FOUND") : (t('status_lost') || "LOST")}
                            </div>
                        ...
                        <div className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-xl space-y-4">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-orange-50 rounded-xl text-orange-500"><Hash className="w-4 h-4" /></div>
                                <div>
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-xl overflow-hidden">
                            <div className="flex items-center justify-between mb-4">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                    <MapPin className="w-4 h-4 text-orange-500" /> {t('last_seen_location') || "LOCATION"}
                                </label>
                            </div>
                            <div className="mb-4 text-[10px] font-bold text-gray-500 bg-gray-50 p-3 rounded-xl border border-gray-100 flex items-start gap-2">
                                <Info className="w-3.5 h-3.5 text-orange-300 mt-0.5" />
                                ...
                                {report.latitude && report.longitude && (
                                    <MapContainer center={[report.latitude, report.longitude]} zoom={15} style={{ height: "100%", width: "100%" }}>
                                        ...
                                        <Marker position={[report.latitude, report.longitude]}>
                                            <Popup>{getLocalizedTitle(report) || "Lost Pet"}</Popup>
                                        </Marker>
                ...
                <div className="space-y-6">
                    <h2 className="text-xl font-black text-gray-900 flex items-center gap-3 px-4">
                        ...
                        {t('linked_found_reports') || "Linked Found Reports"}
                        <span className="text-xs font-black text-gray-300 bg-gray-100 px-2 py-1 rounded-lg">
                            {report.foundReports?.length || 0}
                        </span>
                    </h2>

                    {report.foundReports && report.foundReports.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {report.foundReports.map((found) => (
                                <div key={found.id} className="bg-white rounded-[2rem] border border-gray-100 shadow-lg overflow-hidden flex cursor-default pointer-events-none">
                                    <div className="w-1/3 aspect-square bg-gray-100 overflow-hidden relative">
                                        {found.imageUrl ? (
                                            <img src={found.imageUrl} className="w-full h-full object-cover" alt="Found" />
                                        ) : (
                                            <div className="w-full h-full flex items-center justify-center text-gray-200"><Dog className="w-8 h-8" /></div>
                                        )}
                                        <div className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-emerald-500 text-white text-[7px] font-black uppercase">
                                            {found.condition}
                                        </div>
                                    </div>
                                    <div className="w-2/3 p-5 flex flex-col justify-between">
                                        <div>
                                            <div className="flex justify-between items-start mb-1">
                                                <h3 className="text-xs font-black text-gray-800 uppercase truncate pr-4">{getLocalizedTitle(found)}</h3>
                                                <span className="text-[8px] font-black text-gray-300">ID: #{found.id}</span>
                                            </div>
                                            <p className="text-[10px] text-gray-500 line-clamp-2 italic mb-3">"{getLocalizedDescription(found)}"</p>
                                        </div>
                                        <div className="flex items-center gap-1.5 text-[9px] font-bold text-emerald-600 border-t border-gray-50 pt-3">
                                            <Calendar className="w-3 h-3" />
                                            {new Date(found.foundDate).toLocaleDateString()}
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-[2.5rem] p-10 text-center">
                            ...
                            <p className="text-xs font-black text-gray-400 uppercase tracking-widest">{t('no_linked_reports_yet') || "No linked reports"}</p>
                        </div>
                    )}
                </div>
            </main>
...
export default AdminViewLostReport;
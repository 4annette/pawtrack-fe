...
const LocationName = ({ lat, lng }) => {
        ...
        const fetchAddress = async () => {
            try {
                ...
                if (data.address) {
                    ...
                    const shortAddr = [addr.road || addr.suburb || addr.neighbourhood, addr.city || addr.town || addr.village].filter(Boolean).join(", ");
    ...
    return <p className="text-[10px] font-bold text-indigo-600 truncate border-b border-indigo-200 border-dashed inline-block max-w-full">{address}</p>;
};
...
const MapModal = ({ isOpen, onClose, lat, lng, title }) => {
    ...
    return (
        <div className="fixed inset-0 z-[10001] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            <div className="bg-white w-full max-w-2xl rounded-[2.5rem] overflow-hidden shadow-2xl relative animate-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
                <button onClick={onClose} className="absolute top-4 right-4 z-10 p-2 bg-white/90 backdrop-blur-md rounded-full hover:bg-red-50 hover:text-red-500 transition-all shadow-md"><X className="w-5 h-5" /></button>
                <div className="p-6 border-b border-gray-100 flex items-center justify-between pr-16">
                    <h3 className="font-black text-gray-900 uppercase tracking-tight flex items-center gap-2"><MapPin className="w-5 h-5 text-orange-500" />{title}</h3>
                    <button onClick={() => { navigator.clipboard.writeText(`${lat}, ${lng}`); toast.success("Coordinates copied!"); }} className="flex items-center gap-1.5 text-[10px] font-black text-gray-400 uppercase hover:text-indigo-600 transition-colors"><Copy className="w-3 h-3" /> {lat.toFixed(4)}, {lng.toFixed(4)}</button>
                </div>
                <div className="h-[450px] w-full bg-gray-50">
                    <iframe title="map-location" width="100%" height="100%" frameBorder="0" src={`https://maps.google.com/maps?q=${lat},${lng}&z=15&output=embed`} allowFullScreen></iframe>
                </div>
            </div>
        </div>
    );
...
const UserLostReports = () => {
    ...
    useEffect(() => { loadReports(); }, [userId, page]);

    const loadReports = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/admin/users/${userId}/lost-reports`, { params: { page, size: 6, sortBy: 'id', sortDirection: 'DESC' } });
        ...
        } catch (error) { toast.error(t('load_reports_failed')); } finally { setLoading(false); }
    };
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 font-sans text-gray-900">
            ...
            <main className="container mx-auto px-4 py-8">
                <div className="mb-8 flex items-end justify-between">
                    <div>
                        <button onClick={() => navigate(-1)} className="group flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-indigo-600 transition-colors mb-4"><ChevronLeft className="w-4 h-4 transition-transform group-hover:-translate-x-1" />{t('back_to_users')}</button>
                        <h1 className="text-3xl font-black text-gray-900 flex items-center gap-3"><FileSearch className="w-8 h-8 text-orange-500" />{t('lost_reports')}</h1>
                        <p className="text-gray-500 mt-2 font-black uppercase text-[10px] tracking-widest">{t('user_id')}: {userId} • {totalElements} {t('reports_total')}</p>
                    </div>
                </div>

                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20"><Loader2 className="w-10 h-10 text-indigo-500 animate-spin" /></div>
                ) : reports.length === 0 ? (
                    <div className="bg-white rounded-[3rem] border border-gray-100 p-20 text-center shadow-sm"><AlertCircle className="w-12 h-12 text-gray-200 mx-auto mb-4" /><h3 className="text-lg font-black text-gray-900 uppercase tracking-widest">{t('no_reports')}</h3></div>
                ) : (
                ...
                {!loading && totalPages > 1 && (
                    <div className="flex items-center justify-between bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm mt-4">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t('page')} {page + 1} {t('of')} {totalPages}</p>
                        <div className="flex gap-2">
                            <button disabled={page === 0} onClick={(e) => { e.stopPropagation(); setPage(p => p - 1); }} className="p-2 border border-gray-200 rounded-xl disabled:opacity-30 hover:bg-gray-50 transition-all shadow-sm"><ChevronLeft className="w-5 h-5" /></button>
                            <button disabled={page >= totalPages - 1} onClick={(e) => { e.stopPropagation(); setPage(p => p + 1); }} className="p-2 border border-gray-200 rounded-xl disabled:opacity-30 hover:bg-gray-50 transition-all shadow-sm"><ChevronRight className="w-5 h-5" /></button>
                        </div>
                    </div>
                )}
            </main>

                            ...
                            <div key={report.id} onClick={() => handleCardClick(report.id)} className="bg-white rounded-[2.5rem] border border-gray-100 p-5 shadow-sm hover:shadow-md transition-all group overflow-hidden relative cursor-pointer">
                                ...
                                <div className="flex flex-col md:flex-row gap-6">
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                                        ...
                                        <div className="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between"><div className="flex items-center gap-3"><div className={`w-9 h-9 rounded-full bg-${activeTab === 'lost' ? 'orange' : 'emerald'}-50 flex items-center justify-center text-${activeTab === 'lost' ? 'orange' : 'emerald'}-600 font-bold text-xs uppercase border border-${activeTab === 'lost' ? 'orange' : 'emerald'}-100 shadow-sm`}>{report.creator?.firstName?.charAt(0) || <User className="w-4 h-4" />}</div><div className="flex flex-col"><span className="text-xs font-bold text-gray-900 leading-none">{report.creator?.firstName} {report.creator?.lastName}</span><span className="text-[10px] text-gray-400 font-medium flex items-center gap-1 mt-1"><Mail className="w-3 h-3" /> {report.creator?.email}</span></div></div><span className="text-[10px] font-black text-gray-300 uppercase tracking-tighter">REF: #{report.id}</span></div>
                                    </div>
                                </div>
                            </div>

        ...
        <div className="bg-gradient-to-br from-emerald-50 via-white to-emerald-50 rounded-[32px] sm:rounded-[40px] shadow-xl border border-emerald-100 p-5 sm:p-10 mb-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              {!isEditing ? (
                <>
                  <button onClick={() => setIsEditing(true)} className="flex-1 sm:flex-none justify-center bg-white border border-emerald-200 text-emerald-600 px-4 py-2.5 rounded-xl text-xs font-black uppercase flex items-center gap-2 hover:bg-emerald-50 transition-all shadow-sm active:scale-95">
          ...
          <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10">
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between p-4 bg-white rounded-2xl border border-emerald-100 shadow-sm">
                <span className="text-xs font-black text-emerald-800 uppercase tracking-widest flex items-center gap-2"><CheckCircle className="w-4 h-4" /> {t('label_found_question', { format: 'uppercase' })}</span>
            ...
            <div className="space-y-5">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block">{t('label_report_title', { format: 'uppercase' })}</label>
                {isEditing ? <input type="text" className="w-full p-3.5 rounded-2xl border border-emerald-100 text-sm font-bold outline-none bg-white shadow-sm focus:ring-4 focus:ring-emerald-500/5 transition-all" value={report.title || ""} onChange={e => setReport({ ...report, title: e.target.value })} /> : <div className="w-full p-3.5 rounded-2xl border border-emerald-50 bg-emerald-50/10 text-sm font-bold text-gray-700 truncate">{report.title || t('untitled')}</div>}
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block">{t('label_description', { format: 'uppercase' })}</label>
                {isEditing ? <textarea className="w-full p-3.5 rounded-2xl border border-emerald-100 text-sm font-bold h-24 resize-none outline-none bg-white shadow-sm" value={report.description || ""} onChange={e => setReport({ ...report, description: e.target.value })} /> : <div className="w-full p-3.5 rounded-2xl border border-emerald-50 bg-emerald-50/10 text-sm font-bold text-gray-700 min-h-[60px] whitespace-pre-wrap">{report.description || t('no_description_provided')}</div>}
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block"><MapPin className="inline w-3 h-3 mr-1" /> {t('label_last_seen_location', { format: 'uppercase' })}</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block">{t('label_chip_number', { format: 'uppercase' })}</label>
                {isEditing ? <input type="number" className="w-full p-3.5 rounded-2xl border border-emerald-100 text-sm font-bold outline-none bg-white shadow-sm" value={report.chipNumber || ""} onChange={e => setReport({ ...report, chipNumber: e.target.value })} /> : <div className="w-full p-3.5 rounded-2xl border border-emerald-50 bg-emerald-50/10 text-sm font-bold text-gray-700">{report.chipNumber || t('not_provided')}</div>}
              </div>

              <CustomDateTimePicker label={t('label_date_lost')} value={report.lostDate || report.createdAt} />
              {isEditing && <div className="flex justify-end pt-6"><button type="submit" disabled={saving} className="w-full sm:w-auto bg-emerald-600 text-white font-black px-10 py-3.5 rounded-2xl hover:bg-emerald-700 transition-all shadow-lg active:scale-95 text-xs uppercase tracking-widest flex items-center justify-center gap-2">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} {t('save_btn', { format: 'uppercase' })}</button></div>}
            </div>
          </form>

          {report.latitude && report.longitude && (
            <div className="mt-10 border-t border-emerald-100 pt-8">
              <h3 className="text-[10px] font-black text-emerald-800 uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><MapPin className="w-4 h-4" /> {t('last_seen_map_title', { format: 'uppercase' })}</h3>
          ...
          <div className="bg-white w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden relative border border-emerald-100 animate-in zoom-in-95 max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="overflow-y-auto p-6 sm:p-8 custom-scrollbar">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div><h2 className="text-lg font-black text-emerald-900 leading-tight">{selectedFoundReport.title}</h2><span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">{t('found_report_preview', { format: 'uppercase' })}</span></div>
              </div>
              ...
              <div className="space-y-4 mb-6">
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-emerald-50/50 rounded-xl border border-emerald-50"><p className="text-[9px] font-black text-emerald-800 uppercase tracking-widest mb-0.5">{t('label_condition', { format: 'uppercase' })}</p><p className="font-bold text-xs text-gray-700 capitalize">{selectedFoundReport.condition || t('unknown')}</p></div>
                  <div className="p-3 bg-emerald-50/50 rounded-xl border border-emerald-50"><p className="text-[9px] font-black text-emerald-800 uppercase tracking-widest mb-0.5">{t('label_found_on', { format: 'uppercase' })}</p><p className="font-bold text-xs text-gray-700">{new Date(selectedFoundReport.foundDate).toLocaleDateString()}</p></div>
                </div>
              ...
              {selectedFoundReport.latitude && selectedFoundReport.longitude && (
                <div className="space-y-3"><div className="flex items-center gap-2 text-[9px] font-black text-emerald-800 uppercase tracking-widest"><MapPin className="w-3.5 h-3.5" /> {t('label_found_location', { format: 'uppercase' })}</div>

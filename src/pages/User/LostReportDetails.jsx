...
import {
  ...
  CheckCircle, MapPin, Clock, ChevronDown, Info, Save, AlertCircle
} from "lucide-react";
    ...
    const timerId = setTimeout(() => {
      ...
      fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${report.latitude}&lon=${report.longitude}`, {
        ...
        .then(json => {
          ...
          setAddressText([city, country].filter(Boolean).join(", ") || "Location available on map below");
        })
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-gray-900">
      <header className="sticky top-0 z-[100] w-full bg-white/80 backdrop-blur-md border-b border-gray-100 shadow-sm h-16 flex items-center px-4">
        <div className="w-full flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="hidden xs:block"><PawTrackLogo size="sm" /></div>
          </div>
          <div className="flex items-center gap-2 sm:gap-4"><Notifications /><ProfileButton /></div>
        </div>
      </header>
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 py-6 sm:py-8">
        <div className="bg-gradient-to-br from-emerald-50 via-white to-emerald-50 rounded-[32px] sm:rounded-[40px] shadow-xl border border-emerald-100 p-5 sm:p-10 mb-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              {!isEditing ? (
                <>
                  <button onClick={() => setIsEditing(true)} className="flex-1 sm:flex-none justify-center bg-white border border-emerald-200 text-emerald-600 px-4 py-2.5 rounded-xl text-xs font-black uppercase flex items-center gap-2 hover:bg-emerald-50 transition-all shadow-sm active:scale-95"><Edit3 className="w-4 h-4" /> Edit</button>
                  <button onClick={handleDeleteReport} className="flex-1 sm:flex-none justify-center bg-red-50 text-red-500 px-4 py-2.5 rounded-xl text-xs font-black uppercase flex items-center gap-2 hover:bg-red-100 transition-colors shadow-sm active:scale-95"><Trash2 className="w-4 h-4" /> Delete</button>
                </>
              ) : (
                <button onClick={handleCancel} className="w-full sm:w-auto justify-center bg-white border border-gray-200 text-gray-500 px-5 py-2.5 rounded-xl text-xs font-black uppercase flex items-center gap-2 hover:bg-gray-50 transition-all shadow-sm"><X className="w-4 h-4" /> Cancel</button>
              )}
            </div>
          </div>
          ...
          <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10">
            <div className="space-y-6">
              ...
              <div className="relative aspect-square rounded-[32px] overflow-hidden border-4 border-white shadow-2xl bg-gray-100">
                {newImage ? <img src={URL.createObjectURL(newImage)} className="w-full h-full object-cover" alt="New Preview" /> : report.imageUrl ? <img src={report.imageUrl} className="w-full h-full object-cover" alt="Report" /> : <div className="w-full h-full flex flex-col items-center justify-center text-gray-300"><ImageIcon className="w-12 h-12" /><span className="text-[10px] font-black uppercase mt-2">No Image</span></div>}
              </div>
              ...
              {isEditing && (
                <div className="space-y-3">
                  ...
                  {report.imageUrl && <button type="button" onClick={handleRemoveImage} className="w-full py-1 text-[10px] font-black text-red-400 uppercase tracking-widest hover:text-red-600 transition-colors">Delete current photo</button>}
                </div>
              )}
            ...
            <div className="space-y-5">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block">Title</label>
                {isEditing ? <input type="text" className="w-full p-3.5 rounded-2xl border border-emerald-100 text-sm font-bold outline-none bg-white shadow-sm focus:ring-4 focus:ring-emerald-500/5 transition-all" value={report.title || ""} onChange={e => setReport({ ...report, title: e.target.value })} /> : <div className="w-full p-3.5 rounded-2xl border border-emerald-50 bg-emerald-50/10 text-sm font-bold text-gray-700 truncate">{report.title || "Untitled"}</div>}
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block">Description</label>
                {isEditing ? <textarea className="w-full p-3.5 rounded-2xl border border-emerald-100 text-sm font-bold h-24 resize-none outline-none bg-white shadow-sm" value={report.description || ""} onChange={e => setReport({ ...report, description: e.target.value })} /> : <div className="w-full p-3.5 rounded-2xl border border-emerald-50 bg-emerald-50/10 text-sm font-bold text-gray-700 min-h-[60px] whitespace-pre-wrap">{report.description || "No description provided."}</div>}
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block"><MapPin className="inline w-3 h-3 mr-1" /> Last Seen Location</label>
                <div className="w-full p-3.5 rounded-2xl border border-emerald-50 bg-emerald-50/20 text-sm font-bold text-gray-400 flex items-center gap-2 cursor-not-allowed overflow-hidden"><Info className="w-3.5 h-3.5 text-emerald-300 shrink-0" /><span className="truncate">{addressText}</span></div>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-emerald-800 uppercase tracking-widest block">Chip Number</label>
                {isEditing ? <input type="number" className="w-full p-3.5 rounded-2xl border border-emerald-100 text-sm font-bold outline-none bg-white shadow-sm" value={report.chipNumber || ""} onChange={e => setReport({ ...report, chipNumber: e.target.value })} /> : <div className="w-full p-3.5 rounded-2xl border border-emerald-50 bg-emerald-50/10 text-sm font-bold text-gray-700">{report.chipNumber || "Not provided"}</div>}
              </div>

              <CustomDateTimePicker label="Date Lost" value={report.lostDate || report.createdAt} />
              {isEditing && <div className="flex justify-end pt-6"><button type="submit" disabled={saving} className="w-full sm:w-auto bg-emerald-600 text-white font-black px-10 py-3.5 rounded-2xl hover:bg-emerald-700 transition-all shadow-lg active:scale-95 text-xs uppercase tracking-widest flex items-center justify-center gap-2">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} SAVE</button></div>}
            </div>
          </form>

          {report.latitude && report.longitude && (
            <div className="mt-10 border-t border-emerald-100 pt-8">
              <h3 className="text-[10px] font-black text-emerald-800 uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><MapPin className="w-4 h-4" /> Last Seen Map</h3>
              <div className="h-64 sm:h-72 w-full rounded-2xl overflow-hidden border border-emerald-200 shadow-sm relative z-0">
                <MapContainer center={[report.latitude, report.longitude]} zoom={15} style={{ height: "100%", width: "100%", zIndex: 0 }}>
                  ...
                  <Marker position={[report.latitude, report.longitude]}><Popup>Last seen here</Popup></Marker>
        ...
        <div className="max-w-2xl mx-auto w-full bg-white rounded-[32px] border border-gray-100 p-6 shadow-sm mb-12">
          <h3 className="text-xs font-black text-emerald-800 uppercase tracking-widest mb-6 flex items-center gap-2">
            <CheckCircle className="w-4 h-4" /> Linked Found Reports
          </h3>
            ...
            {report.foundReports && report.foundReports.length > 0 ? (
              report.foundReports.map((fr) => (
                <div
                  ...
                  className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100 cursor-pointer hover:bg-emerald-100/50 transition-all group flex items-center justify-between gap-4"
                >
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-sm text-emerald-900 truncate">{fr.title}</p>
              ...
              <div className="py-8 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                <p className="text-xs text-gray-400 italic font-medium">No linked found reports yet</p>
              </div>
            )}
          </div>
        </div>
      ...
      {selectedFoundReport && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in" onClick={() => setSelectedFoundReport(null)}>
          <div className="bg-white w-full max-w-md rounded-[40px] shadow-2xl overflow-hidden relative border border-emerald-100 animate-in zoom-in-95 max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="overflow-y-auto p-6 sm:p-8 custom-scrollbar">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-2.5 bg-emerald-100 rounded-xl"><Dog className="w-5 h-5 text-emerald-600" /></div>
                <div><h2 className="text-lg font-black text-emerald-900 leading-tight">{selectedFoundReport.title}</h2><span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Found Report Preview</span></div>
              </div>
              <div className="aspect-video w-full rounded-2xl bg-gray-50 border-4 border-white shadow-md overflow-hidden mb-6">
                {selectedFoundReport.imageUrl ? <img src={selectedFoundReport.imageUrl} className="w-full h-full object-cover" alt="Pet" /> : <div className="w-full h-full flex flex-col items-center justify-center text-gray-300"><ImageIcon className="w-10 h-10" /><p className="text-[9px] font-black uppercase mt-1">No Photo</p></div>}
              </div>
              <div className="space-y-4 mb-6">
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-emerald-50/50 rounded-xl border border-emerald-50"><p className="text-[9px] font-black text-emerald-800 uppercase tracking-widest mb-0.5">Condition</p><p className="font-bold text-xs text-gray-700 capitalize">{selectedFoundReport.condition || "Unknown"}</p></div>
                  <div className="p-3 bg-emerald-50/50 rounded-xl border border-emerald-50"><p className="text-[9px] font-black text-emerald-800 uppercase tracking-widest mb-0.5">Found On</p><p className="font-bold text-xs text-gray-700">{new Date(selectedFoundReport.foundDate).toLocaleDateString()}</p></div>
                </div>
                <div className="p-4 bg-gray-50 rounded-xl"><p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Description</p><p className="text-xs text-gray-600 font-medium leading-relaxed">{selectedFoundReport.description || "No description provided."}</p></div>
              </div>
              {selectedFoundReport.latitude && selectedFoundReport.longitude && (
                <div className="space-y-3"><div className="flex items-center gap-2 text-[9px] font-black text-emerald-800 uppercase tracking-widest"><MapPin className="w-3.5 h-3.5" /> Found Location</div>
